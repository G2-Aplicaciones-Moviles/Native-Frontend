...
fun LunchScreen(navController: NavController) {
    val recipes = listOf(
        ...
        "Ceviche saludable",
        "Sudado de pescado con yuca sancochada",
    ...
    LazyColumn(
        ...
        item {
            Text("Almuerzo", fontWeight = FontWeight.Bold, fontSize = 20.sp)
            Spacer(Modifier.height(16.dp))
        }

        items(recipes) { title ->
            RecipeItem(title) {
                navController.navigate(RecipeRoute.LUNCH_DETAIL)
            }
        ...
        item {
            ...
            NutritionInfoItem("Valor Energético", "900 kcal", R.drawable.info_nutri)
            Spacer(Modifier.height(16.dp))
            NutritionInfoItem("Proteínas", "120g", R.drawable.info_nutri)
        }
    }
}
...
@Composable
fun NutritionInfoItem(title: String, value: String, iconRes: Int) {
    Row(
        verticalAlignment = Alignment.CenterVertically,
